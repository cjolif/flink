...
public class ElasticsearchSinkITCase extends ElasticsearchSinkTestBase {
	...
	public void runTransportClientTest() throws Exception {
		...
		source.addSink(createElasticSearchSinkRestforEmbeddededNode(userConfig,
			new WrapBaseSinkFunction(new SourceSinkDataTestKit.TestElasticsearchSinkFunction(index))));
	...
	public void runNullTransportClientTest() throws Exception {
		try {
			Map<String, String> userConfig = new HashMap<>();
			userConfig.put(ElasticsearchSinkBase.CONFIG_KEY_BULK_FLUSH_MAX_ACTIONS, "1");
			createElasticsearchSinkRest(userConfig, null, new WrapBaseSinkFunction<>(new SourceSinkDataTestKit
	...
	public void runEmptyTransportClientTest() throws Exception {
		try {
			Map<String, String> userConfig = new HashMap<>();
			userConfig.put(ElasticsearchSinkBase.CONFIG_KEY_BULK_FLUSH_MAX_ACTIONS, "1");
			createElasticsearchSinkRest(userConfig,
				Collections.<HttpHost>emptyList(),
				new WrapBaseSinkFunction<>(new SourceSinkDataTestKit.TestElasticsearchSinkFunction("test")));
		} catch (IllegalArgumentException expectedException) {
			// test passes
			return;
		}
	...
	@Override
	protected <T> ElasticsearchSinkBase<T> createElasticsearchSink(Map<String, String> userConfig, List<InetSocketAddress>
		transportAddresses, org.apache.flink.streaming.connectors.elasticsearch.ElasticsearchSinkFunction<T> elasticsearchSinkFunction) {
		return null;
	}

	@Override
	protected <T> ElasticsearchSinkBase<T> createElasticsearchSinkForEmbeddedNode(Map<String, String> userConfig, org.apache.flink.streaming.connectors.elasticsearch.ElasticsearchSinkFunction<T>
		elasticsearchSinkFunction) throws Exception {
		return null;
	}

	private <T> ElasticsearchSink<T> createElasticSearchSinkRestforEmbeddededNode(Map<String, String> userConfig, ElasticsearchSinkFunction elasticsearchSinkFunction) {
		ArrayList<HttpHost> httpHosts = new ArrayList<>();
		httpHosts.add(new HttpHost("127.0.0.1", 9200, "http"));
		return new ElasticsearchSink<>(userConfig, httpHosts, elasticsearchSinkFunction);
	}

	private <T> ElasticsearchSink<T> createElasticsearchSinkRest(
